...
# modular convelutional blocks
def conv_bn_pool_block(inputs, filters, conv_kernel, conv_padding, block_num, activation='relu', batch_norm=True, pooling=True, pool_kernel=(2, 2)):
    ...
    return x
# a function to build the basic CNN needed for command classification
def build_CNN(input_shape, num_classes, main_activation = 'relu'):
    input_tensor = Input(input_shape)
    x = input_tensor

    x = conv_bn_pool_block(x , 32, (3, 3), 'same', '1', main_activation, True, False)
